...
export type DanmuDensity = 'dense' | 'medium' | 'sparse';
...
export const DANMU_DENSITY_OPTIONS = ['dense', 'medium', 'sparse'] as const;
...
export const sanitizeDanmuDensity = (value: unknown): DanmuDensity => {
  if (value === 'dense' || value === 'medium' || value === 'sparse') {
    return value;
  }
  return 'medium';
};

export const getDanmuDensityLabel = (value: DanmuDensity): string => {
  if (value === 'dense') {
    return '密集';
  }
  if (value === 'sparse') {
...
export const getDanmuDensityInterval = (value: DanmuDensity): number => {
  if (value === 'dense') {
    return 0;
  }
  if (value === 'sparse') {
  ...
  try {
    ...
    return {
      ...
      settings: {
        ...
        density: sanitizeDanmuDensity(settings.density),
      },
    };
  } catch (error) {
