...
const DOUYU_DANMAKU_RECONNECT_LIMIT = 2;
...
export async function getDouyuStreamConfig(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      const signData = await invoke<string>('sign_douyu_request', { roomId: realRoomId });
      ...
      console.error('Diagnostic: playerHelper.ts:115 (details omitted)');
...
export async function startDouyuDanmakuListener(
  ...
  const connectDanmakuSocket = async () => {
    ...
    socket.onerror = async (error) => {
      ...
      } catch (emitError) { console.warn('Diagnostic: playerHelper.ts:263 (details omitted)'); }
      postAndroidDebugLog('e', `[DouyuDanmaku] socket error room=${normalizedRoomId} ${String(error)}`);
      console.warn('Diagnostic: playerHelper.ts:265 (details omitted)');
    };
...
export async function stopDouyuProxy(): Promise<void> {
  ...
  try {
    ...
    console.error('Diagnostic: playerHelper.ts:304 (details omitted)');
  } finally {
...
async function resolveDouyuDanmakuRoomId(roomId: string): Promise<string> {
  try {
    ...
    console.warn('Diagnostic: playerHelper.ts:370 (details omitted)');
  }
  return roomId;
}
...
function pushDouyuDanmakuToUi(
  ...
  if (shouldDisplay && activeOverlay?.sendComment) {
    try {
      ...
      console.warn('Diagnostic: playerHelper.ts:565 (details omitted)');
    }
  }
...
function startNativeDouyuDanmakuListener(
  ...
  const eventHandler = (event: Event) => {
    ...
    try {
      ...
      console.warn('Diagnostic: playerHelper.ts:598 (details omitted)');
    }
  };
  ...
  const statusHandler = async (event: Event) => {
    try {
      ...
      console.warn('Diagnostic: playerHelper.ts:609 (details omitted)');
    }
  };
