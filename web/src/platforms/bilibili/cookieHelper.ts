...
export const ensureBilibiliLoginWindow = async (): Promise<{ label: string }> => {
  try {
    const mod = await import('@tauri-apps/api/webviewWindow');
    const existing = await mod.WebviewWindow.getByLabel(BILIBILI_LOGIN_WINDOW_LABEL);
    if (existing) {
      try {
        await existing.show();
    ...
    const loginWindow = new mod.WebviewWindow(BILIBILI_LOGIN_WINDOW_LABEL, {
