...
import { exportConfigWithBrowser, importConfigWithBrowser, openExternal } from './runtime/host';
...
const handleExportConfig = async () => {
  ...
  try {
    ...
    const fileName = buildPortableConfigFileName();
    await exportConfigWithBrowser(fileName, JSON.stringify(payload, null, 2));
    configStatus.value = { tone: 'success', text: `閰嶇疆宸插鍑轰负 ${fileName}` };
  } catch (error) {
...
const handleImportConfig = async () => {
  if (isImportingConfig.value || typeof window === 'undefined' || !window.localStorage) return;

  isImportingConfig.value = true;
  configStatus.value = { tone: 'info', text: '请选择要导入的配置文件...' };
  try {
    const imported = await importConfigWithBrowser();
    if (!imported) {
    ...
    configStatus.value = { tone: 'success', text: '已导入 ' + imported.path + '，正在刷新界面。' };
    window.setTimeout(() => window.location.reload(), 360);
  } catch (error) {
    console.error('[App] Import config failed:', error);
    configStatus.value = { tone: 'error', text: '导入失败，文件格式可能不正确。' };
  } finally {
...
const handleCheckUpdate = async () => {
  ...
  updateMessage.value = '';
  try {
    const result = await checkLatestRelease(appVersion.value || '0.0.0');
    if (result.hasUpdate) {
      updateMessage.value = '发现新版本 ' + result.latestVersion + '，将打开 Releases 页面。';
      await openExternal(result.htmlUrl || RELEASES_PAGE);
    } else {
      updateMessage.value = '当前已经是最新版本 ' + (result.currentVersion || appVersion.value || '4.0.0') + '。';
    }
  } catch (error) {
