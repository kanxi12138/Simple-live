...
const sanitizeFollowedStreamers = (streamers: FollowedStreamer[]): FollowedStreamer[] => {
  return streamers.filter((streamer) => !isRemovedPlatform((streamer as any)?.platform));
};

const sanitizeFolders = (folders: FollowFolder[]): FollowFolder[] => {
  return folders.map((folder) => ({
    ...folder,
    streamerIds: Array.isArray(folder.streamerIds)
      ? folder.streamerIds.filter((streamerId) => {
          const platformKey = typeof streamerId === 'string' ? streamerId.split(':', 1)[0] : '';
          return !isRemovedPlatform(platformKey);
        })
      : [],
  }));
};

const sanitizeListOrder = (items: FollowListItem[]): FollowListItem[] => {
  const sanitized: FollowListItem[] = [];
  items.forEach((item) => {
    if (item.type === 'streamer') {
      if (!isRemovedPlatform((item.data as any)?.platform)) {
        sanitized.push(item);
      }
      return;
    }
    sanitized.push({
      ...
      data: {
        ...item.data,
        streamerIds: Array.isArray(item.data.streamerIds)
          ? item.data.streamerIds.filter((streamerId) => {
              const platformKey = typeof streamerId === 'string' ? streamerId.split(':', 1)[0] : '';
              return !isRemovedPlatform(platformKey);
            })
          : [],
