...
pub(super) async fn runtime_client(cookie: Option<&str>) -> Result<(reqwest::Client, String)> {
    let bootstrap = reqwest::Client::builder().no_proxy().build()?;
    ...
    Ok((reqwest::Client::builder().no_proxy().default_headers(headers).build()?, cookie))
}
...
async fn play_info(client: &reqwest::Client, room_id: &str, quality: Option<i32>) -> Result<Value> {
    ...
    let response = client.get(PLAY_ENDPOINT).query(&parameters).send().await?
