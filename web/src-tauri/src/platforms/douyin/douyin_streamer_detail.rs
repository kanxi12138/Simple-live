...
pub(crate) fn collect_available_streams(room: &Value) -> Option<Vec<StreamVariant>> {
    ...
    if let Some(hls_map) = stream_url.get("hls_pull_url_map").and_then(|v| v.as_object()) {
        variants.extend(hls_map.iter().filter_map(|(k, v)| {
    ...
    if let Some(flv_map) = stream_url.get("flv_pull_url").and_then(|v| v.as_object()) {
        variants.extend(flv_map.iter().filter_map(|(k, v)| {
            v.as_str().map(|url| StreamVariant {
                url: url.to_string(),
