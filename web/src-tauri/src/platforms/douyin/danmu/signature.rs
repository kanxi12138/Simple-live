...
use quick_js::Context as QuickJsContext;
use rand::Rng;
...
#[cfg(not(target_os = "android"))]
fn evaluate_sign_js(md5_param: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
...
#[cfg(target_os = "android")]
fn evaluate_sign_js(md5_param: &str) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    let context = QuickJsContext::new().map_err(|e| format!("Failed to create quick-js context: {e}"))?;
    let bootstrap_script = r#"
        globalThis.window = globalThis;
        globalThis.self = globalThis;
        globalThis.document = {};
    ...
    context
        ...
        .map_err(|e| format!("Error during quick-js bootstrap script: {e}"))?;
    context
