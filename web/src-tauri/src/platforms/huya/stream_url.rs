...
use super::cdn_token::{fetch_token, sign_anti_code, PLAY_USER_AGENT};
...
pub(super) async fn fetch_room_page(client: &reqwest::Client, room_id: &str) -> Result<RoomPage> {
    ...
    let html = client.get(format!("https://m.huya.com/{room_id}"))
        .header("User-Agent", MOBILE_USER_AGENT).send().await?
...
async fn build_entries(client: &reqwest::Client, page: &RoomPage) -> Result<Vec<HuyaUnifiedStreamEntry>> {
    ...
    for line in lines {
        ...
        if !tokens.contains_key(stream) {
            let token = fetch_token(client, stream).await?;
...
pub async fn get_huya_unified_cmd(
    ...
    let entries = if is_live {
        build_entries(client, &page).await.map_err(|error| error.to_string())?
    } else { Vec::new() };
