...
impl StreamSession {
    ...
    pub(crate) fn new(source: StreamSource) -> Self {
        Self { source, id: rand::random::<u64>().to_string(), resources: Arc::default() }
    }
    ...
    fn register_resource(&self, base: &Url, path: &str) -> Result<String, actix_web::Error> {
        let url = base.join(path).map_err(actix_web::error::ErrorBadGateway)?;
        ...
        let key = resources.iter().find(|(_, existing)| *existing == &url)
            .map(|(key, _)| key.clone()).unwrap_or_else(|| resources.len().to_string());
...
pub(crate) async fn forward_stream(
    ...
    let url = if let Some(resource) = &query.resource {
        ...
        Url::parse(&session.source.url).map_err(actix_web::error::ErrorBadGateway)?
    };
    ...
    let response = upstream.send().await.map_err(actix_web::error::ErrorBadGateway)?;
    let status = actix_web::http::StatusCode::from_u16(response.status().as_u16())
        .map_err(actix_web::error::ErrorBadGateway)?;
    if !status.is_success() { return Ok(HttpResponse::build(status).finish()); }
    ...
    if is_manifest {
        ...
        let manifest = response.text().await.map_err(actix_web::error::ErrorBadGateway)?;
    ...
    Ok(builder.streaming(response.bytes_stream().map_err(actix_web::error::ErrorBadGateway)))
}
