...
async fn fetch_room_from_api(
    ...
    let json: Value = http_client
        ...
        .send_limited()
        .await
        .map_err(|_| "抖音网络请求失败".to_string())?
        .error_for_status().map_err(|_| "抖音平台拒绝请求，暂不可用".to_string())?
        .json()
        .await
        .map_err(|e| format!("Failed to parse Douyin web enter response: {}", e))?;

    if json.get("status_code").and_then(Value::as_i64).is_some_and(|code| code != 0) {
        return Err("抖音平台限制访问，暂不可用".into());
    }
    let room = json
        .get("data")
...
pub async fn fetch_room_data(
    ...
    fetch_room_from_api(http_client, &actual_web_id, Some(&cookie)).await
}
