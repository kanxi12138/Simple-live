...
pub async fn connect_and_manage_websocket(
    ...
    let (ws_stream, _response) = match connect_async(client_request).await {
        ...
        Err(_) => {
            log::warn!("Diagnostic: websocket_connection.rs:69 (details omitted)");
    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = ticker.tick() => {
                    if let Err(_) = write.send(ws_ping_msg.clone()).await {
                        println!("Diagnostic: websocket_connection.rs:102 (details omitted)");
                        break;
                    }
                }
                msg_opt = rx.recv() => {
                    match msg_opt {
                        Some(msg_to_send) => {
                            if let Err(e) = write.send(msg_to_send).await {
                                println!("Diagnostic: websocket_connection.rs:110 (details omitted)");
                                if matches!(
                                    e,
        ...
        println!("Diagnostic: websocket_connection.rs:126 (details omitted)");
    });
