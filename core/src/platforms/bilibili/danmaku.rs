...
async fn run_listener(room: &str, cookie: Option<&str>, app: &tauri::AppHandle) -> Result<()> {
    ...
    for host in hosts {
        ...
        match connect_async(format!("wss://{hostname}:{port}/sub")).await {
            ...
            Err(error) => log::warn!("Bilibili danmaku server failed: {error}"),
        }
    }
        ...
        tokio::select! {
            ...
            result = run_listener(&room, cookie.as_deref(), &app_handle) => {
                if let Err(error) = result {
                    log::error!("Bilibili danmaku: {error}");
                    if let Err(emit_error) = app_handle.emit("danmaku-status", json!({"room_id": room,
