    ...
    fn parse_business_message(&mut self, h: MsgHead, b: &[u8]) -> Option<BiliMessage> {
        ...
        if h.operation == 5 {
            if h.ver == 3 {
                ...
                let res: Vec<u8> = match decompress_zlib(b) {
                    Ok(r) => r,
                    Err(e) => {
                        ws_debug!("[websocket] zlib decompress error: {:?}", e);
                        return None;
                    }
                };
                ws_debug!("[websocket] zlib decompressed len={}", res.len());
                return self.parse_ws_message(res);
            } else if h.ver == 0 {
...
pub fn connect(v: Value) -> WebSocket<MaybeTlsStream<TcpStream>> {
    let danmu_server = gen_damu_list(&v);
    let (_host, _url, ws_url) = find_server(danmu_server);
    ws_debug!("[websocket] connecting ws {}", ws_url);
    let (socket, _resp) =
        ws_connect(Url::parse(ws_url.as_str()).unwrap()).expect("Can't connect");
...
pub fn make_packet(body: &str, ops: Operation) -> Vec<u8> {
    ...
    let pack_len: [u8; 4] = ((16 + body_content.len()) as u32).to_be_bytes();
...
pub fn decompress_zlib(body: &[u8]) -> std::io::Result<Vec<u8>> {
    use flate2::read::ZlibDecoder;
    use std::io::Read;
    let mut decoder = ZlibDecoder::new(body);
    let mut decoded_input = Vec::new();
    let _ = decoder.read_to_end(&mut decoded_input)?;
    Ok(decoded_input)
}
...
pub fn handle(json: Value) -> Option<BiliMessage> {
    ...
    match category {
        cmd if cmd.starts_with("DANMU_MSG") => Some(BiliMessage::Danmu {
            user: json["info"][2][1]
                .as_str()
                .unwrap_or("<unknown>")
                .to_string(),
        ...
        cmd if cmd.starts_with("SEND_GIFT") => Some(BiliMessage::Gift {
            user: json["data"]["uname"]
                .as_str()
                .unwrap_or("<unknown>")
                .to_string(),
            gift: format!(
                "{} x{}",
                json["data"]["giftName"].as_str().unwrap_or("礼物"),
                json["data"]["num"].as_i64().unwrap_or(1)
            ),
        }),
        cmd if cmd.starts_with("INTERACT_WORD") => Some(BiliMessage::Activity {
            user: json["data"]["uname"]
                .as_str()
                .unwrap_or("<unknown>")
                .to_string(),
        ...
        cmd if cmd.starts_with("ENTRY_EFFECT") => Some(BiliMessage::Activity {
            user: json["data"]["copy_writing_v2"]
                .as_str()
                .or_else(|| json["data"]["copy_writing"].as_str())
                .unwrap_or("进入直播间")
                .to_string(),
            text: String::new(),
        }),
