...
fn init_buvid(headers: HeaderMap) -> Option<String> {
    let client = reqwest::blocking::Client::builder()
        .https_only(true)
        .no_proxy()
        .build()
    ...
    let mut request_headers = headers;
    request_headers.insert("user-agent", USER_AGENT.parse().ok()?);
    ...
    let buvid = response
        .cookies()
        .find(|cookie| cookie.name().eq_ignore_ascii_case("buvid3"))
        .map(|cookie| cookie.value().to_string());
    buvid
}

fn init_room_id(headers: HeaderMap, room_id: &str) -> Option<String> {
    let client = reqwest::blocking::Client::builder()
        .https_only(true)
        .no_proxy()
        .build()
    ...
    let json: Value = serde_json::from_str(body.as_str()).ok()?;
    json["data"]["room_id"]
        .as_u64()
...
pub const BUVID_INIT_URL: &str = "https://www.bilibili.com/";
pub const ROOM_INIT_URL: &str = "https://api.live.bilibili.com/room/v1/Room/room_init";
...
fn build_auth_context(headers: HeaderMap, room_id: &str, uid: &str) -> (Value, AuthMessage) {
    let mut auth_map = HashMap::new();
    auth_map.insert("uid".to_string(), uid.to_string());

    let actual_room_id = init_room_id(headers.clone(), room_id).unwrap_or_else(|| room_id.to_string());
    eprintln!(
        "[Bilibili Danmaku] Requested room {} resolved to actual room {}",
        room_id, actual_room_id
    );

    if let Some(buvid) = init_buvid(headers.clone()) {
        eprintln!("[Bilibili Danmaku] Resolved buvid3 for room {}", actual_room_id);
        auth_map.insert("buvid".to_string(), buvid);
    } else {
        eprintln!(
            "[Bilibili Danmaku] Failed to resolve buvid3 for room {}, continue without it",
            actual_room_id
        );
    }

    auth_map.insert("room_id".to_string(), actual_room_id.clone());

    let room_id_num = actual_room_id
        .parse::<u64>()
        .expect("room_id must be a valid u64");
    let (_, body4) = init_host_server(headers.clone(), room_id_num);
    let body4_res: Value = serde_json::from_str(body4.as_str()).unwrap();
    let server_info = body4_res["data"].clone();
    let token = body4_res["data"]["token"]
        .as_str()
        .expect("token should exist in getDanmuInfo response");
    eprintln!(
        "[Bilibili Danmaku] getDanmuInfo succeeded for room {}",
        actual_room_id
    );
    auth_map.insert("token".to_string(), token.to_string());

    let auth_msg = AuthMessage::from(&auth_map);
    (server_info, auth_msg)
}
...
pub fn init_server_with_cookie(cookies: &str, room_id: &str) -> (Value, AuthMessage) {
    ...
    let uid = body1_v["data"]["mid"]
        .as_i64()
        .map(|value| value.to_string())
        .unwrap_or_else(|| "0".to_string());
    build_auth_context(headers, room_id, &uid)
}
...
pub fn init_server_no_cookie(room_id: &str) -> (Value, AuthMessage) {
    ...
    build_auth_context(headers, room_id, "0")
}
